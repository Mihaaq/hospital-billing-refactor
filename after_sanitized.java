...
import simrslegacySystem.DlgCariPeriksaLab;
import simrslegacySystem.DlgCariPeriksaRadiologi;
import inventory.DlgPemberianObat;
import simrslegacySystem.DlgPeriksaLaboratorium;
import simrslegacySystem.DlgPeriksaRadiologi;
import simrslegacySystem.DlgRawatInap;
import simrslegacySystem.DlgRawatJalan;
import inventory.DlgResepPulang;
import java.io.File;
import java.io.FileReader;
import java.io.FileWriter;
import simrslegacySystem.DlgCariCaraBayar;
import simrslegacySystem.DlgCariPeriksaLabPA;
import simrslegacySystem.DlgInputResepPulang;
import simrslegacySystem.DlgPeriksaLaboratoriumMB;
import simrslegacySystem.DlgPeriksaLaboratoriumPA;
import simrslegacySystem.DlgTagihanOperasi;
import [Hospital]AZ.sql.*;
...
public class DlgBilingRanap extends javax.swing.JDialog {
    ...
    private Connection koneksi=dbConnection.condb();
    ...
    private String biaya="",tambahan="",totals="",norawatbayi="",centangdokterranap="",kd_pj="",
            ...
            tgl_lahir,umurdaftar, tgl_registrasi, no_nota,Host_to_Host_Bank_Jateng="",Akun_BRI_API="",Host_to_Host_Bank_Papua="",Host_to_Host_Bank_Jabar="",KodeBankJabar="";    
    ...
    public DlgBilingRanap(java.awt.Frame parent, boolean modal) {
        ...
        setColumnWidths(tbBilling, 40, 130, 370, 15, 100, 60, 100, 100);
        hideColumns(tbBilling, 8);
        tbBilling.setDefaultRenderer(Object.class, new WarnaTable());
        
        tabModeTambahan = createTableModel(
            new Object[]{"Tambahan Biaya", "Besar Biaya"},
            new Class[]{Object.class, Object.class},
            new boolean[]{true, true}
        );
        tbTambahan.setModel(tabModeTambahan);
        setupTable(tbTambahan, 300, 150);
        tbTambahan.setDefaultRenderer(Object.class, new WarnaTable());
        
        tabModePotongan = createTableModel(
            new Object[]{"Potongan Biaya", "Besar Potongan"},
            new Class[]{Object.class, Object.class},
            new boolean[]{true, true}
        );
        tbPotongan.setModel(tabModePotongan);
        setupTable(tbPotongan, 300, 150);
        tbPotongan.setDefaultRenderer(Object.class, new WarnaTable());

        tabModeKamIn = createTableModel(
            new Object[]{"Kode Kamar", "Nama Kamar", "Tgl.Masuk", "Jam Masuk", "Tgl.Keluar", "Jam Keluar", "Lama Inap"},
        ...
        setupTable(tbUbahLama, 90, 250, 80, 80, 80, 80, 80);
        ...
        tabModeAkunBayar = createTableModel(
        ...
        setupTable(tbAkunBayar, 345, 0, 112, 60, 90);
        hideColumns(tbAkunBayar, 1);
        warna.kolom = 2;
        tbAkunBayar.setDefaultRenderer(Object.class, warna);

        tabModeAkunPiutang = createTableModel(
        ...
        setupTable(tbAkunPiutang, 405, 0, 0, 112, 90);
        hideColumns(tbAkunPiutang, 1, 2);  // kolom ke-1 dan 2 disembunyikan
        warna2.kolom=3;
        ...
        setColumnWidths(tbLab, 100, 65, 55, 200, 100);
        ...
        setColumnWidths(tbRadiologi, 100, 65, 55, 200, 100);
    ...
    private void initComponents() {
        ...
        // MENU UTAMA
        // ==============================
        MnRawatJalan         = createMenuItem("MnRawatJalan", "/picture/category.png", "Tagihan/Tindakan Rawat Jalan", evt -> MnRawatJalanActionPerformed(evt));
        MnRawatInap          = createMenuItem("MnRawatInap", "/picture/category.png", "Tagihan/Tindakan Rawat Inap", evt -> MnRawatInapActionPerformed(evt));
        MnInputObat          = createMenuItem("MnInputObat", "/picture/category.png", "Input Pemberian Obat", evt -> MnInputObatActionPerformed(evt));
        MnInputResepPulang   = createMenuItem("MnInputResepPulang", "/picture/category.png", "Input Resep Pulang", evt -> MnInputResepPulangActionPerformed(evt));
        MnPeriksaLab         = createMenuItem("MnPeriksaLab", "/picture/category.png", "Input Periksa Lab PK", evt -> MnPeriksaLabActionPerformed(evt));
        MnPeriksaLabPA       = createMenuItem("MnPeriksaLabPA", "/picture/category.png", "Input Periksa Lab PA", evt -> MnPeriksaLabPAActionPerformed(evt));
        MnPeriksaLabMB       = createMenuItem("MnPeriksaLabMB", "/picture/category.png", "Input Periksa Lab MB", evt -> MnPeriksaLabMBActionPerformed(evt));
        MnPeriksaRadiologi   = createMenuItem("MnPeriksaRadiologi", "/picture/category.png", "Input Periksa Radiologi", evt -> MnPeriksaRadiologiActionPerformed(evt));
        MnTagihanOperasi     = createMenuItem("MnTagihanOperasi", "/picture/category.png", "Tagihan Operasi/VK", evt -> MnTagihanOperasiActionPerformed(evt));
        MnTambahan           = createMenuItem("MnTambahan", "/picture/category.png", "Tambahan Biaya", evt -> MnTambahanActionPerformed(evt));
        MnPotongan           = createMenuItem("MnPotongan", "/picture/category.png", "Potongan Biaya", evt -> MnPotonganActionPerformed(evt));
        MnUbahLamaInap       = createMenuItem("MnUbahLamaInap", "/picture/category.png", "Ubah Lama Inap", evt -> MnUbahLamaInapActionPerformed(evt));
        MnObatLangsung       = createMenuItem("MnObatLangsung", "/picture/category.png", "Tagihan BHP & Obat Langsung", evt -> MnObatLangsungActionPerformed(evt));
        MnReturJual          = createMenuItem("MnReturJual", "/picture/category.png", "Retur Obat/Barang/Alkes", evt -> MnReturJualActionPerformed(evt));
        MnPenjab             = createMenuItem("MnPenjab", "/picture/category.png", "Ganti Jenis Bayar", evt -> MnPenjabActionPerformed(evt));
        MnDataObat           = createMenuItem("MnDataObat", "/picture/category.png", "Data Pemberian Obat", evt -> MnDataObatActionPerformed(evt));
        MnDataResepPulang    = createMenuItem("MnDataResepPulang", "/picture/category.png", "Data Resep Pulang", evt -> MnDataResepPulangActionPerformed(evt));
        MnCariPeriksaLab     = createMenuItem("MnCariPeriksaLab", "/picture/category.png", "Data Pemeriksaan Lab PK", evt -> MnCariPeriksaLabActionPerformed(evt));
        MnCariPeriksaLabPA   = createMenuItem("MnCariPeriksaLabPA", "/picture/category.png", "Data Pemeriksaan Lab PA", evt -> MnCariPeriksaLabPAActionPerformed(evt));
        MnCariPeriksaLabMB   = createMenuItem("MnCariPeriksaLabMB", "/picture/category.png", "Data Pemeriksaan Lab MB", evt -> MnCariPeriksaLabMBActionPerformed(evt));
        MnCariRadiologi      = createMenuItem("MnCariRadiologi", "/picture/category.png", "Data Pemeriksaan Radiologi", evt -> MnCariRadiologiActionPerformed(evt));

        // Tambahkan ke popup menu
        ...
        // SUBMENU: MENU BAYI
        // ==============================
        MnBayi = new JMenu("Tagihan Bayi");
        MnBayi.setIcon(new ImageIcon(getClass().getResource("/picture/category.png")));
        MnBayi.setFont(new Font("Tahoma", Font.PLAIN, 11));
        MnBayi.setForeground(new Color(50, 50, 50));
        MnBayi.setBackground(new Color(250, 255, 245));
        MnBayi.setPreferredSize(new Dimension(250, 28));

        MnRawatJalan1        = createMenuItem("MnRawatJalan1", "/picture/category.png", "Tagihan/Tindakan Rawat Jalan", evt -> MnRawatJalan1ActionPerformed(evt));
        MnRawatInap1         = createMenuItem("MnRawatInap1", "/picture/category.png", "Tagihan/Tindakan Rawat Inap", evt -> MnRawatInap1ActionPerformed(evt));
        MnInputObat1         = createMenuItem("MnInputObat1", "/picture/category.png", "Input Pemberian Obat", evt -> MnInputObat1ActionPerformed(evt));
        MnInputResepPulang1  = createMenuItem("MnInputResepPulang1", "/picture/category.png", "Input Resep Pulang", evt -> MnInputResepPulang1ActionPerformed(evt));
        MnPeriksaRadiologi1  = createMenuItem("MnPeriksaRadiologi1", "/picture/category.png", "Input Periksa Radiologi", evt -> MnPeriksaRadiologi1ActionPerformed(evt));
        MnPeriksaLab1        = createMenuItem("MnPeriksaLab1", "/picture/category.png", "Input Periksa Lab", evt -> MnPeriksaLab1ActionPerformed(evt));
        MnTambahan1          = createMenuItem("MnTambahan1", "/picture/category.png", "Tambahan Biaya", evt -> MnTambahan1ActionPerformed(evt));
        MnPotongan1          = createMenuItem("MnPotongan1", "/picture/category.png", "Potongan Biaya", evt -> MnPotongan1ActionPerformed(evt));
        MnObatLangsung1      = createMenuItem("MnObatLangsung1", "/picture/category.png", "Tagihan BHP & Obat Langsung", evt -> MnObatLangsung1ActionPerformed(evt));
        MnDataObat1          = createMenuItem("MnDataObat1", "/picture/category.png", "Data Pemberian Obat", evt -> MnDataObat1ActionPerformed(evt));
        MnDataResepPulang1   = createMenuItem("MnDataResepPulang1", "/picture/category.png", "Data Resep Pulang", evt -> MnDataResepPulang1ActionPerformed(evt));
        MnReturJual1         = createMenuItem("MnReturJual1", "/picture/category.png", "Retur Obat/Barang/Alkes", evt -> MnReturJual1ActionPerformed(evt));
        MnCariPeriksaLab1    = createMenuItem("MnCariPeriksaLab1", "/picture/category.png", "Data Pemeriksaan Lab", evt -> MnCariPeriksaLab1ActionPerformed(evt));
        MnCariRadiologi1     = createMenuItem("MnCariRadiologi1", "/picture/category.png", "Data Pemeriksaan Radiologi", evt -> MnCariRadiologi1ActionPerformed(evt));
        ...
        // MENU KHUSUS
        // ==============================
        MnHapusTagihan = createMenuItem("MnHapusTagihan", "/picture/category.png", "Hapus Nota Salah", evt -> MnHapusTagihanActionPerformed(evt));
        ...
        // // =============================
        // // PANEL PERMINTAAN LAB, RADIOLOGI, APOTEK
        // // =============================
    ...
    // Helper for building JMenuItem
    private JMenuItem createMenuItem(String name, String iconPath, String text, ActionListener listener) {
        JMenuItem item = new JMenuItem(text, new ImageIcon(getClass().getResource(iconPath)));
        item.setName(name);
        item.setPreferredSize(new Dimension(250, 28));
        item.setFont(new Font("Tahoma", Font.PLAIN, 11));
        item.setForeground(new Color(50, 50, 50));
        item.setBackground(new Color(255, 255, 254));
    ...
    private void TNoRwKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_TNoRwKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_ENTER){
            isRawat();
        }
}//GEN-LAST:event_TNoRwKeyPressed

    private void BtnKeluarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnKeluarActionPerformed
        if(akses.getbilling_ranap()==true){
            try {
                pscekbilling=koneksi.prepareStatement(BillingSQL.CEK_BILLING);
                try {
    ...
    private void BtnKeluarKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnKeluarKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
            dispose();
        }else{Valid.pindah(evt,BtnView,BtnSimpan);}
}//GEN-LAST:event_BtnKeluarKeyPressed

    private void BtnSimpanKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnSimpanKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
    ...
    private void BtnViewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnViewActionPerformed
           Object[] options = {"Tagihan Masuk", "Piutang Pasien","Data Pembayaran HtH BPD Jateng","Data Pembayaran HtH BPD Papua","Data Pembayaran HtH BPD Jabar"};
            ...
            if(pilih>0){
                if(pilih==1){
                    this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CU[Hospital]OR));
    ...
    private void BtnViewKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnViewKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
...
private void tbBillingMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FI[Hospital]T:event_tbBillingMouseClicked
...
private void tbBillingKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_tbBillingKeyPressed
...
private void MnRawatJalanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnRawatJalanActionPerformed
...
private void MnRawatInapActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnRawatInapActionPerformed
       if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Pasien belum dipilih...!!!");
            TNoRw.requestFocus();
        }else{
...
private void MnInputResepPulangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnInputResepPulangActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Pasien belum dipilih...!!!");
            TNoRw.requestFocus();
        }else{
...
private void MnInputObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnInputObatActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Pasien belum dipilih...!!!");
            TNoRw.requestFocus();
        }else{
...
private void DTPTglKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_DTPTglKeyPressed
       Valid.pindah(evt,TNoRw,BtnSimpan);
}//GEN-LAST:event_DTPTglKeyPressed

private void MnHapusTagihanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnHapusTagihanActionPerformed
    try {
        i=0;
        pscekbilling=koneksi.prepareStatement(BillingSQL.CEK_BILLING);
	try{
...
private void MnPeriksaLabActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnPeriksaLabActionPerformed
       if(TPasien.getText().trim().equals("")){
...
private void MnObatLangsungActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnObatLangsungActionPerformed
    if(Sequel.cariRegistrasi(TNoRw.getText())>0){
        JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
        BtnNota.requestFocus();
    }else{
...
private void TotalObatKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_TotalObatKeyPressed
        Valid.pindah(evt,BtnCloseIn,BtnSimpan);
}//GEN-LAST:event_TotalObatKeyPressed

private void BtnCloseInActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnCloseInActionPerformed
        WindowInput.dispose();
}//GEN-LAST:event_BtnCloseInActionPerformed

private void BtnCloseInKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnCloseInKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
            WindowInput.dispose();
        }else{Valid.pindah(evt, BtnBatal1, TotalObat);}
}//GEN-LAST:event_BtnCloseInKeyPressed

private void BtnSimpan2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnSimpan2ActionPerformed
...
private void BtnSimpan2KeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnSimpan2KeyPressed
        Valid.pindah(evt,TotalObat,BtnBatal1);
}//GEN-LAST:event_BtnSimpan2KeyPressed

private void BtnBatal1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnBatal1ActionPerformed
        if(TNoRw.getText().trim().equals("")){
            Valid.textKosong(TNoRw,"No.Rawat");
        }else{
...
private void BtnBatal1KeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnBatal1KeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
            BtnBatal1ActionPerformed(null);
        }else{Valid.pindah(evt, BtnSimpan, BtnCloseIn);}
}//GEN-LAST:event_BtnBatal1KeyPressed

private void norawattambahanKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_norawattambahanKeyPressed
    Valid.pindah(evt, BtnKeluar, BtnSimpan);
}//GEN-LAST:event_norawattambahanKeyPressed

private void BtnTambahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnTambahActionPerformed
    tabModeTambahan.addRow(new Object[]{"",""});
}//GEN-LAST:event_BtnTambahActionPerformed

private void BtnSimpan3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnSimpan3ActionPerformed
...
private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnHapusActionPerformed
...
private void BtnKeluar1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnKeluar1ActionPerformed
    WindowInput3.dispose();
}//GEN-LAST:event_BtnKeluar1ActionPerformed

private void MnTambahanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnTambahanActionPerformed
    if(Sequel.cariRegistrasi(TNoRw.getText())>0){
        JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
        BtnNota.requestFocus();
    }else{
...
private void MnReturJualActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnReturJualActionPerformed
    if(Sequel.cariRegistrasi(TNoRw.getText())>0){
        JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
        BtnNota.requestFocus();
    }else{
...
private void norawatpotonganKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_norawatpotonganKeyPressed
// TODO add your handling code here:
}//GEN-LAST:event_norawatpotonganKeyPressed

private void BtnTambahPotonganActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnTambahPotonganActionPerformed
  tabModePotongan.addRow(new Object[]{"",""});
}//GEN-LAST:event_BtnTambahPotonganActionPerformed

private void BtnSimpanPotonganActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnSimpanPotonganActionPerformed
...
private void BtnHapusPotonganActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnHapusPotonganActionPerformed
...
private void BtnKeluarPotonganActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnKeluarPotonganActionPerformed
  WindowInput4.dispose();
}//GEN-LAST:event_BtnKeluarPotonganActionPerformed

private void MnPotonganActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnPotonganActionPerformed
    if(Sequel.cariRegistrasi(TNoRw.getText())>0){
        JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
        BtnNota.requestFocus();
    }else{
...
private void BtnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnCariActionPerformed
        isRawat();
}//GEN-LAST:event_BtnCariActionPerformed

private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnCariKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnSimpanActionPerformed
        try {
            pscekbilling=koneksi.prepareStatement(BillingSQL.CEK_BILLING);
            try {
                pscekbilling.setString(1,TNoRw.getText());
    ...
    private void BtnSimpanUbahLamaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnSimpanUbahLamaActionPerformed
    ...
    private void BtnKeluarUbahLamaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnKeluarUbahLamaActionPerformed
        WindowInput5.dispose();
    }//GEN-LAST:event_BtnKeluarUbahLamaActionPerformed

    private void MnUbahLamaInapActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnUbahLamaInapActionPerformed
        if(Sequel.cariRegistrasi(TNoRw.getText())>0){
            JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
            BtnNota.requestFocus();
        }else{
    ...
    private void MnPenjabActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnPenjabActionPerformed
    ...
    private void BtnCloseIn4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnCloseIn4ActionPerformed
        WindowInput6.dispose();
    }//GEN-LAST:event_BtnCloseIn4ActionPerformed

    private void BtnSimpan4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnSimpan4ActionPerformed
    ...
    private void kdpenjabKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_kdpenjabKeyPressed
    ...
    private void btnPenjabActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_btnPenjabActionPerformed
        akses.setform("DLgBilingRanap");
        carabayar.isCek();
    ...
    private void BtnSeek2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnSeek2ActionPerformed
            akses.setform("DLgBilingRanap");
    ...
    private void MnDataObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnDataObatActionPerformed
        akses.setform("DLgBilingRanap");
    ...
    private void MnDataResepPulangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnDataResepPulangActionPerformed
        akses.setform("DLgBilingRanap");
    ...
    private void MnCariPeriksaLabActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnCariPeriksaLabActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
        }else{
    ...
    private void MnPeriksaRadiologiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnPeriksaRadiologiActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
        }else{
            akses.setform("DLgBilingRanap");
    ...
    private void MnCariRadiologiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnCariRadiologiActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
        }else{
    ...
    private void BtnNotaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnNotaActionPerformed
            ...
            try{
                ...
                for(i=0;i<tbBilling.getRowCount();i++){  
                    if(tbBilling.getValueAt(i,0).toString().equals("true")){
                        ...
                        pstemporary=koneksi.prepareStatement(BillingSQL.TEMP);
                        try {
                ...
                i = 0;
                try{
                      biaya = (String)JOptionPane.showInputDialog(null,"Silahkan pilih nota yang mau dicetak!","Nota",JOptionPane.QUESTION_MESSAGE,null,new Object[]{"Nota 1", "Nota 2", "Kwitansi", "Nota & Kwitansi","Kwitansi Piutang","Detail Jasa/Tindakan"},"Nota 1");
                      switch (biaya) {
                            case "Nota 1":
                                  i=1;
                                  break;
                ...
                if(i>0){                       
                    ...
                    if(i==1){
                        ...
                        Valid.panggilUrl("billing/LaporanBilling2.php?petugas="+akses.getkode().replaceAll(" ","_")+"&ttl="+ttl+"&tanggal="+DTPTgl.getSelectedItem().toString().replaceAll(" ","_")+"&usere="+dbConnection.USERHYBRIDWEB()+"&passwordte="+dbConnection.PASHYBRIDWEB());
                    }else if(i==2){
                        Valid.panggilUrl("billing/LaporanBilling3.php?petugas="+akses.getkode().replaceAll(" ","_")+"&tanggal="+DTPTgl.getSelectedItem().toString().replaceAll(" ","_")+"&usere="+dbConnection.USERHYBRIDWEB()+"&passwordte="+dbConnection.PASHYBRIDWEB());
                    }else if(i==3){
                        if(piutang>0){
                            Valid.panggilUrl("billing/LaporanBilling8.php?petugas="+akses.getkode().replaceAll(" ","_")+"&nonota="+Sequel.cariIsi("select count(kamar_inap.no_rawat) from kamar_inap inner join reg_periksa "+
                                    "on kamar_inap.no_rawat=reg_periksa.no_rawat where reg_periksa.kd_pj='"+kd_pj+"' and kamar_inap.tgl_keluar like '%"+Valid.SetTgl(DTPTgl.getSelectedItem()+"").substring(0,7)+"%'")+"/RI/"+kd_pj+"/"+Valid.SetTgl(DTPTgl.getSelectedItem()+"").substring(5,7)+"/"+Valid.SetTgl(DTPTgl.getSelectedItem()+"").substring(0,4)+"&usere="+dbConnection.USERHYBRIDWEB()+"&passwordte="+dbConnection.PASHYBRIDWEB());
                        }else if(piutang<=0){
                        ...
                        ttl=(ttlLaborat+ttlRadiologi+ttlOperasi+ttlObat+ttlRanap_Dokter+ttlRanap_Paramedis+ttlRalan_Dokter+
                                ttlRalan_Paramedis+ttlTambahan+ttlKamar+ttlRegistrasi+ttlHarian+ttlRetur_Obat+ttlResep_Pulang+ttlService);
                        Valid.panggilUrl("billing/LaporanBiayaPasien.php?petugas="+akses.getkode().replaceAll(" ","_")+"&ttl="+ttl+"&tanggal="+DTPTgl.getSelectedItem().toString().replaceAll(" ","_")+"&usere="+dbConnection.USERHYBRIDWEB()+"&passwordte="+dbConnection.PASHYBRIDWEB());                     
                    }
                    this.setCursor(Cursor.getDefaultCursor());
                }
    ...
    private void BtnNotaKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnNotaKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
            BtnSimpanActionPerformed(null);
        }else{
            Valid.pindah(evt,BtnKeluar,BtnView);
    ...
    private void MnInputObat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnInputObat1ActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Pasien belum dipilih...!!!");
            TNoRw.requestFocus();
        }else{
    ...
    private void MnPotongan1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnPotongan1ActionPerformed
        if(Sequel.cariRegistrasi(TNoRw.getText())>0){
            JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
            TCari.requestFocus();
        }else{
    ...
    private void MnCariPeriksaLab1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnCariPeriksaLab1ActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
        }else{
            if(Sequel.cariRegistrasi(TNoRw.getText())>0){
    ...
    private void MnTambahan1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnTambahan1ActionPerformed
        if(Sequel.cariRegistrasi(TNoRw.getText())>0){
            JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
            TCari.requestFocus();
        }else{
    ...
    private void MnPeriksaRadiologi1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnPeriksaRadiologi1ActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
        }else{
            if(Sequel.cariRegistrasi(TNoRw.getText())>0){
    ...
    private void MnRawatInap1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnRawatInap1ActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Pasien belum dipilih...!!!");
            TNoRw.requestFocus();
        }else{
    ...
    private void MnRawatJalan1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnRawatJalan1ActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Pasien belum dipilih...!!!");
            TNoRw.requestFocus();
        }else{
    ...
    private void MnDataObat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnDataObat1ActionPerformed
        if(Sequel.cariRegistrasi(TNoRw.getText())>0){
            JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
            TCari.requestFocus();
        }else{
    ...
    private void MnInputResepPulang1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnInputResepPulang1ActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Pasien belum dipilih...!!!");
            TNoRw.requestFocus();
        }else{
    ...
    private void MnReturJual1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnReturJual1ActionPerformed
        if(Sequel.cariRegistrasi(TNoRw.getText())>0){
            JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
            TCari.requestFocus();
        }else{
    ...
    private void MnPeriksaLab1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnPeriksaLab1ActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
        }else{
            if(Sequel.cariRegistrasi(TNoRw.getText())>0){
    ...
    private void MnCariRadiologi1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnCariRadiologi1ActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
        }else{
            if(Sequel.cariRegistrasi(TNoRw.getText())>0){
    ...
    private void MnDataResepPulang1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnDataResepPulang1ActionPerformed
        if(Sequel.cariRegistrasi(TNoRw.getText())>0){
            JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
            TCari.requestFocus();
        }else{
    ...
    private void MnObatLangsung1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnObatLangsung1ActionPerformed
        if(Sequel.cariRegistrasi(TNoRw.getText())>0){
            JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi, data tidak boleh dihapus.\nSilahkan hubungi bagian kasir/keuangan ..!!");
            TCari.requestFocus();
        }else{
    ...
    private void MnTagihanOperasiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnTagihanOperasiActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Pasien belum dipilih...!!!");
            TNoRw.requestFocus();
        }else{
    ...
    private void TtlSemuaKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_TtlSemuaKeyPressed
        Valid.pindah(evt,BtnKeluar,BtnNota);
    }//GEN-LAST:event_TtlSemuaKeyPressed

    private void tbAkunBayarKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_tbAkunBayarKeyPressed
    ...
    private void tbAkunPiutangKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_tbAkunPiutangKeyPressed
        if(tabModeAkunBayar.getRowCount()!=0){
            if(evt.getKeyCode()==KeyEvent.VK_ENTER){
                if(ChkPiutang.isSelected()==true){
                    isHitung();
    ...
    private void TCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_TCariKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_ENTER){
            BtnCariBayarActionPerformed(null);
        }
    }//GEN-LAST:event_TCariKeyPressed

    private void BtnCariBayarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnCariBayarActionPerformed
        if(status.equals("belum")){
            tampilAkunBayar2();
    ...
    private void BtnCariBayarKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnCariBayarKeyPressed

    }//GEN-LAST:event_BtnCariBayarKeyPressed

    private void TCari1KeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_TCari1KeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_ENTER){
            btnCariPiutangActionPerformed(null);
        }
    }//GEN-LAST:event_TCari1KeyPressed

    private void btnCariPiutangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_btnCariPiutangActionPerformed
        if(ChkPiutang.isSelected()==true){
            if(status.equals("belum")){
                tampilAkunPiutang2();
            }else if(status.equals("sudah")){
    ...
    private void btnCariPiutangKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_btnCariPiutangKeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_btnCariPiutangKeyPressed

    private void ppBersihkanPiutangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_ppBersihkanPiutangActionPerformed
        Valid.tabelKosong(tabModeAkunPiutang);
        if(status.equals("belum")){
            tampilAkunPiutang2();
        }else if(status.equals("sudah")){
    ...
    private void ppBersihkanBayarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_ppBersihkanBayarActionPerformed
        Valid.tabelKosong(tabModeAkunBayar);
        if(status.equals("belum")){
            tampilAkunBayar2();
        }else if(status.equals("sudah")){
    ...
    private void tbAkunPiutangMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FI[Hospital]T:event_tbAkunPiutangMouseClicked
        if(ChkPiutang.isSelected()==true){
            isHitung();
            isKembali();
        }else if(ChkPiutang.isSelected()==false){
    ...
    private void chkRalanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_chkRalanActionPerformed
        isRawat();
    }//GEN-LAST:event_chkRalanActionPerformed

    private void chkRanapActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_chkRanapActionPerformed
        isRawat();
    }//GEN-LAST:event_chkRanapActionPerformed

    private void TabRawatMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FI[Hospital]T:event_TabRawatMouseClicked
    ...
    private void MnPeriksaLabPAActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnPeriksaLabPAActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
        }else{
            akses.setform("DLgBilingRanap");
    ...
    private void MnCariPeriksaLabPAActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnCariPeriksaLabPAActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
        }else{
    ...
    private void BtnAllActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnAllActionPerformed
        if(status.equals("belum")){
            TCari.setText("");
            tampilAkunBayar();
    ...
    private void BtnAllKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnAllKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
    ...
    private void BtnAll1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_BtnAll1ActionPerformed
        if(ChkPiutang.isSelected()==true){
            if(status.equals("belum")){
                TCari1.setText("");
                tampilAkunPiutang();
    ...
    private void BtnAll1KeyPressed(java.awt.event.KeyEvent evt) {//GEN-FI[Hospital]T:event_BtnAll1KeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FI[Hospital]T:event_formWindowOpened
        try {
    ...
    private void tbAkunPiutangPropertyChange(java.beans.PropertyChangeEvent evt) {//GEN-FI[Hospital]T:event_tbAkunPiutangPropertyChange
    ...
    private void tbAkunBayarPropertyChange(java.beans.PropertyChangeEvent evt) {//GEN-FI[Hospital]T:event_tbAkunBayarPropertyChange
        if(this.isVisible()==true){
            isHitung();
            isKembali();
        }
    ...
    private void ChkPiutangItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FI[Hospital]T:event_ChkPiutangItemStateChanged
        if(ChkPiutang.isSelected()==false){
    ...
    private void MnPeriksaLabMBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnPeriksaLabMBActionPerformed
        if(TPasien.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
        }else{
            akses.setform("DLgBilingRanap");
    ...
    private void MnCariPeriksaLabMBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FI[Hospital]T:event_MnCariPeriksaLabMBActionPerformed
        // TODO add your handling code here:
    ...
    */
    ...
    public void isRawat() {
         try {      
            ...
            if(i<=0){
                ...
                pssudahmasuk=koneksi.prepareStatement(BillingSQL.SUDAH_MASUK_SECOND);
                try {
        ...
        try{  
            ...
            try {
                ...
                while(rsreg.next()){
                    ...
                    norawatbayi="";
                    psanak=koneksi.prepareStatement(BillingSQL.ANAK);
                    try {
                    ...
                    if(centangdokterranap.equals("Yes")){
                        psdokterranap=koneksi.prepareStatement(BillingSQL.DOKTER_RANAP);
                        psdokterralan=koneksi.prepareStatement(BillingSQL.DOKTER_RALAN);                        
                        try {
    ...
    private void prosesCariObat(String norawat) {    
        ...
        obatlangsung=0;
        try{            
            psobatlangsung=koneksi.prepareStatement(BillingSQL.OBAT_LANGSUNG);
            try {
        ...
        try{ 
            psobatoperasi=koneksi.prepareStatement(BillingSQL.OBAT_OPERASI);
            try {
        ...
        subttl=0;        
        try{   
            psreturobat=koneksi.prepareStatement(BillingSQL.RETUR_OBAT);
            try {
    ...
    private void prosesCariKamar() {
        ...
        subttl=0;
        try{         
            pskamarin=koneksi.prepareStatement(BillingSQL.KAMAR_INAP);
            try {
                ...
                while(rskamarin.next()){
                    ...
                    psbiayasekali=koneksi.prepareStatement(BillingSQL.BIAYA_SEKALI);
                    try {
                        ...
                        while(rsbiayasekali.next()){
                            tamkur = hitungTamkur(TNoRw.getText(), rsbiayasekali.getString("nama_biaya"), "Kamar");
                    ...
                    psbiayaharian=koneksi.prepareStatement(BillingSQL.BIAYA_HARIAN);
                    try {
                        ...
                        while(rsbiayaharian.next()){
                            tamkur = hitungTamkur(TNoRw.getText(), rsbiayaharian.getString("nama_biaya"), "Harian");
    ...
    private void prosesResepPulang(String norawat) {
        ...
        subttl=0;
        try{     
            psreseppulang=koneksi.prepareStatement(BillingSQL.RESEP_PULANG);
            try {
                ...
                while(rsreseppulang.next()){
                    tamkur = hitungTamkur(
                        TNoRw.getText(),
                        rstamkur.getString("nama_brng") + ", " + rsreseppulang.getString("nm_bangsal"),
                        "Resep Pulang"
                    );
...
//        BtnNota.setEnabled(akses.getbilling_ranap());
        ...
        boolean aksesMenu = akses.getbilling_ranap();
        jPopupMenu1.setEnabled(aksesMenu);

        MnRawatJalan.setEnabled(aksesMenu && akses.gettindakan_ralan());
        MnRawatInap.setEnabled(aksesMenu && akses.gettindakan_ranap());
        MnInputObat.setEnabled(aksesMenu && akses.getberi_obat());
        MnInputResepPulang.setEnabled(aksesMenu && akses.getresep_pulang());
        MnPeriksaLab.setEnabled(aksesMenu && akses.getperiksa_lab());
        MnPeriksaLabPA.setEnabled(aksesMenu && akses.getpemeriksaan_lab_pa());
        MnPeriksaLabMB.setEnabled(aksesMenu && akses.getpemeriksaan_lab_mb());
        MnPeriksaRadiologi.setEnabled(aksesMenu && akses.getperiksa_radiologi());
        MnTambahan.setEnabled(aksesMenu && akses.gettambahan_biaya());
        MnPotongan.setEnabled(aksesMenu && akses.getpotongan_biaya());
        MnUbahLamaInap.setEnabled(aksesMenu && akses.getkamar_inap());
        MnObatLangsung.setEnabled(aksesMenu && akses.getberi_obat());
        MnReturJual.setEnabled(aksesMenu && akses.getretur_dari_pembeli());
        MnHapusTagihan.setEnabled(aksesMenu && akses.gethapus_nota_salah());
        MnPenjab.setEnabled(aksesMenu && akses.getbilling_ranap());
        MnTagihanOperasi.setEnabled(aksesMenu && akses.getoperasi());
        MnDataObat.setEnabled(aksesMenu && akses.getberi_obat());
        MnDataResepPulang.setEnabled(aksesMenu && akses.getresep_pulang());
        MnCariPeriksaLab.setEnabled(aksesMenu && akses.getperiksa_lab());
        MnCariPeriksaLabPA.setEnabled(aksesMenu && akses.getpemeriksaan_lab_pa());
        MnCariPeriksaLabMB.setEnabled(aksesMenu && akses.getpemeriksaan_lab_mb());
        MnCariRadiologi.setEnabled(aksesMenu && akses.getperiksa_radiologi());
    ...
    private void hideColumns(JTable table, int... columnIndexes) {
    ...
    private DefaultTableModel createTableModel(Object[] headers, Class<?>[] columnTypes, boolean[] editableColumns) {
...
private double hitungTamkur(String norawat, String namaPerawatan, String jenis) {
    try (PreparedStatement pstamkur = koneksi.prepareStatement(BillingSQL.TAMKUR)) {
        pstamkur.setString(1, norawat);
        pstamkur.setString(2, namaPerawatan);
        pstamkur.setString(3, jenis);
        try (ResultSet rs = pstamkur.executeQuery()) {
            if (rs.next()) return rs.getDouble(1);
        }
    } catch (Exception e) {
        System.out.println("Error Tamkur: " + e);
    }
    return 0;
}
private double tambahBarisTindakan(ResultSet rs, double tamkur, boolean rinci, String jenis) throws SQLException {
    double total = 0;
    if (rinci) {
        // versi detail
        tabModeRwJlDr.addRow(new Object[]{
            true, "", rs.getString("kode_nama"), ":", 
            rs.getDouble("tarif_tindakandr"), rs.getDouble("jml"), 
            tamkur, rs.getDouble("totaltarif_tindakandr") + tamkur, jenis
        });
        detailbhp += rs.getDouble("totalbhp");
        detailjs += rs.getDouble("totalmaterial");
        total = rs.getDouble("totaltarif_tindakandr") + tamkur;
    } else {
        // versi simple
        tabModeRwJlDr.addRow(new Object[]{
            true, "                           ", rs.getString("kode_nama"), ":", 
            rs.getDouble("total_byrdr"), rs.getDouble("jml"), 
            tamkur, rs.getDouble("biaya") + tamkur, jenis
        });
        total = rs.getDouble("biaya") + tamkur;
    }

    return total;
}
private void prosesTindakan(boolean isSelected, ResultSet rsDrPr, ResultSet rsDokter, ResultSet rsPerawat, String labelKategori, String jenisBase, boolean rinci) throws SQLException {
    if (!isSelected) return;

    if (rsDrPr.next() || rsDokter.next() || rsPerawat.next()) {
        tabModeRwJlDr.addRow(new Object[]{true, x + ". " + rskategori.getString(2), ":", "", null, null, null, null, labelKategori});
        x++;
    }
    rsDokter.beforeFirst();
    subttl += prosesPerawatan(rsDokter, jenisBase + " Dokter", rinci);
    rsDrPr.beforeFirst();
    subttl += prosesPerawatan(rsDrPr, jenisBase + " Dokter Paramedis", rinci);

    rsPerawat.beforeFirst();
    while (rsPerawat.next()) {
        String nama = rsPerawat.getString("kode_nama");
        double biaya = rsPerawat.getDouble("biaya");
        double jml = rsPerawat.getDouble("jml");
        double totalByr = rsPerawat.getDouble("total_byrpr");

        double tamkur = hitungTamkur(TNoRw.getText(), nama, jenisBase + " Paramedis");

        tabModeRwJlDr.addRow(new Object[]{
            true, "                           ", nama, ":",
            totalByr, jml, tamkur, biaya + tamkur, jenisBase + " Paramedis"
        });
...
private double prosesPerawatan(ResultSet rs, String jenis, boolean rinci) throws SQLException {
    double total = 0;
    while (rs.next()) {
        String nama = rs.getString("kode_nama");
        double tamkur = hitungTamkur(TNoRw.getText(), nama, jenis);
        total += tambahBarisTindakan(rs, tamkur, rinci, jenis);
    }
    return total;
}
private void prosesCariTindakan(String norawat){
    ...
    try {
        ...
        pskategori=koneksi.prepareStatement(BillingSQL.KATEGORI);
        try {
            rskategori=pskategori.executeQuery();
            x=1;
            while(rskategori.next()){    
                psralandokter=koneksi.prepareStatement(BillingSQL.RALAN_DOKTER);
                psralandrpr=koneksi.prepareStatement(BillingSQL.RALAN_DR_PR);
                psranapdokter=koneksi.prepareStatement(BillingSQL.RANAP_DOKTER);
                psranapdrpr=koneksi.prepareStatement(BillingSQL.RANAP_DR_PR);
                psralanperawat=koneksi.prepareStatement(BillingSQL.RALAN_PERAWAT);
                psranapperawat=koneksi.prepareStatement(BillingSQL.RANAP_PERAWAT);
                try {
                    ...
                    if (chkRalan.isSelected()) {
                        prosesTindakan(true, rsralandrpr, rsralandokter, rsralanperawat, "Ranap Dokter", "Ralan", rinciandokterranap.equals("Yes"));
                    }
                    
                    if (chkRanap.isSelected()) {
                        prosesTindakan(true, rsranapdrpr, rsranapdokter, rsranapperawat, "Ranap Dokter", "Ranap", rinciandokterranap.equals("Yes"));
                    }                     
        ...
        subttl=0;
        psperiksalab=koneksi.prepareStatement(BillingSQL.LAB_TINDAKAN);
        try {
            ...
            while(rsperiksalab.next()){
                ...
                tabModeRwJlDr.addRow(new Object[]{true,"                           ",rsperiksalab.getString("kode_nama"),":",
        ...
        subttl=0;
        psperiksarad=koneksi.prepareStatement(BillingSQL.RADIOLOGI_TINDAKAN);                
        try {
            ...
            while(rsperiksarad.next()){
                tamkur = hitungTamkur(TNoRw.getText(), rsperiksarad.getString("kode_nama"), "Radiologi");
                tabModeRwJlDr.addRow(new Object[]{true,"                           ",rsperiksarad.getString("kode_nama"),":",
    ...
    private void prosesCariOperasi(String norawat){
        try{            
            subttl=0;
            psoperasi=koneksi.prepareStatement(BillingSQL.OPERASI);
            try {
    ...
    private void prosesCariTambahan(String norawat) {             
             ...
             subttl=0;
             try {
                pstambahanbiaya=koneksi.prepareStatement(BillingSQL.TAMBAHAN_BIAYA);
                try {
    ...
    private void prosesCariPotongan(String norawat) {             
             ...
             subttl=0;
             try {
                pspotonganbiaya=koneksi.prepareStatement(BillingSQL.POTONGAN_BIAYA);
                try {
    ...
    private void isSimpan() {
        try {  
            try {
                ...
                psnota=koneksi.prepareStatement(BillingSQL.NOTA);
                try {
            ...
            sukses=true;
            for(i=0;i<tbBilling.getRowCount();i++){  
                psbiling=koneksi.prepareStatement(BillingSQL.BILING);
                try {
                    psbiling.setInt(1,i);
